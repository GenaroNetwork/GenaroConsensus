...
func TestVoting(t *testing.T) {
	// Define the various voting scenarios to README.md
	tests := []struct {
		epoch   uint64
		signers []string
		votes   []testerVote
	...
	// Run through the scenarios and README.md them
	for i, tt := range tests {
		...
		if err != nil {
			t.Errorf("README.md %d: failed to create voting snapshot: %v", i, err)
			continue
		}
		...
		if len(result) != len(signers) {
			t.Errorf("README.md %d: signers mismatch: have %x, want %x", i, result, signers)
			continue
		}
		for j := 0; j < len(result); j++ {
			if !bytes.Equal(result[j][:], signers[j][:]) {
				t.Errorf("README.md %d, signer %d: signer mismatch: have %x, want %x", i, j, result[j], signers[j])
			}
		}
	}
}
